...
const StatusModalEdit = ({isActive, onSetActive, onFetchStatuses, status}) => {
    ...
    const handleOnCreateTask = (e) => {
        ...
        axios.patch(`http://localhost:4444/status/${status._id}`, {
            title: title,
        }, {
            headers: {
            ...
            <div className={"modal__body"} onClick={e => e.stopPropagation()}>
                <form className={"modal__form"} onSubmit={(e) => handleOnCreateTask(e)}>
                    ...
                    <button type={"submit"} className={"modal__submit"} disabled={!isFormValid}>
                        {
                            isFormValid ? ("Изменить статус") : ("Заполните поля")
                        }
                    </button>
                </form>
            </div>
